  ...
  if (!selectedSquad || !state) {
    return (
      <footer style={footerStyle}>
        <span style={{ color: "var(--text-secondary)" }}>Select a squad to monitor</span>
  ...
  return (
    <footer style={footerStyle}>
      <div style={{ display: "flex", alignItems: "center", gap: 16, flex: 1, minWidth: 0 }}>
        ...
        {state.startedAt && (
          <span style={{ color: "var(--text-secondary)" }}>{formatElapsed(elapsed)}</span>
        )}
        {state.handoff && (
          <span
            style={{
            ...
            title={`${state.handoff.from} -> ${state.handoff.to ?? "pending"}: ${state.handoff.message}`}
          >
            {state.handoff.from}
            {" -> "}
            {state.handoff.to ?? "pending"}: {state.handoff.message}
          </span>
        )}
      </div>
      <ConnectionDot connected={isConnected} />
